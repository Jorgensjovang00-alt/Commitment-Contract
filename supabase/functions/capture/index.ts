...
serve(async (req) => {
  ...
  try {
    ...
    const resp = await fetch(`https://api.stripe.com/v1/payment_intents/${payment_intent_id}/capture`, {
      method: 'POST',
      headers: {
    ...
    return new Response(JSON.stringify({ captured: true }), { headers: { 'Content-Type': 'application/json' } });
  } catch (e) {
